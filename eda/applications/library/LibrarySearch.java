...
public class LibrarySearch {
    ...
    public ListPOI<Term> termsRepeated(int n) {  
        /* TO BE COMPLETED */
        ListPOI<Term> result = new LinkedListPOI<Term>();
        ListPOI<Term> keysList = index.keys();
        
        keysList.begin();
        while(!keysList.isEnd()){
            Term term = keysList.get();
            ListPOI<Posting> postings = index.get(term);
            if(postings != null && postings.size() == n) result.add(term);
            keysList.next();
        }
