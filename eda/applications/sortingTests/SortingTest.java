...
public class SortingTest {
    ...
    public static boolean check() {
        ...
        String[] a1 = createRandomString(100000, 80);
        String[] a2 = Arrays.copyOf(a1, a1.length);
     ...
     * arrays of strings.
     */
    public static void timeString(int prefixLength) {
        final int BEGIN = 10000; // Smallest size of array to be tested
        final int END = 100000; // Largest array tested
        final int INC = BEGIN; // Size increment
        final int numRep = 200; // Number of repetitions per size
        double t1, t2; // These hold the start and end times
        double acc1, acc2, acc3, acc4; // Accumulators for the four sorting methods
        String[] aux1, aux2, aux3, aux4; // Arrays to be filled with random Strings and sorted

        System.out.println("#------------------------------------------------------------");        
        System.out.println("# Comparison between quickSorth, mergeSort1, mergeSort2 and Arrays.sort: ");
        System.out.println("# Timings in milliseconds for Strings - " + prefixLength);
        System.out.println("#------------------------------------------------------------");
        System.out.println("#  Size     mergeSort1   mergeSort2   quickSort   Arrays.sort");
        System.out.println("#------------------------------------------------------------");
        for (int k = BEGIN; k <= END; k = k + INC) {
            int size = k;
            t1 = 0; t2 = 0; 
